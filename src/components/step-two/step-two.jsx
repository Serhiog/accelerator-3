...
import { handleSetTotalPrice, handleFirstPayDeal, handleMotherCapital } from "../../store/action"

const StepTwo = ({ handleSetTotalPrice, handleFirstPayDeal, totalPrice, firstPay, handleMotherCapital }) => {
    ...
    return (
        <>
            ...
            <div className={+firstPay < 30000 ? "calculator__value-wrapper calculator__value-wrapper--error-2" : "calculator__value-wrapper calculator__value-wrapper--first-pay"}>
                <div className="calulator__value-result-wrapper">
                    <input step="100000" type="tel" className="calulator__value-result" value={prettify(firstPay)} onChange={handleFirstPay} />
                    <span className={+firstPay < 30000 ? "calulator__value-no-error calulator__value-error-2" : "calulator__value-no-error "}>Некорректное значение</span>
                    <span className="calulator__value-result-rub"> рублей</span>
                </div>
            </div>
            ...
            <div className="calculator__value-wrapper calculator__value-wrapper--first-pay">
                <p className="calulator__value-result-years">5 лет</p>
            </div>
            <div className="calulator__value-range-wrapper">
                <input type="range" className="calulator__value-range"></input>
...
export default connect(mapStateToProps, { handleSetTotalPrice, handleFirstPayDeal, handleMotherCapital })(StepTwo)